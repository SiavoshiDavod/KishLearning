...
namespace MVC.Controls
{
    public class PagedList<T> : List<T>, IPagedList<T>
    {
        ...
        protected void Initialize(IQueryable<T> source, int index, int pageSize, int? totalCount)
        {
            ...
            {
                index = 100;
                // throw new ArgumentOutOfRangeException("PageIndex cannot be below 0."); Change By ME(Zakarya)
            }
            if (pageSize < 1)
            {
                //throw new ArgumentOutOfRangeException("PageSize cannot be less than 1."); Change By ME(Zakarya)
            }
