...
namespace MVC.Controls.Paging
{
	public class Pager : IHtmlString
	{
		...
		public Pager(HtmlHelper htmlHelper, int pageSize, int currentPage, int totalItemCount,string url)
		{
		...
		private string GeneratePageUrl(int pageNumber)
		{
            ...
            if (!string.IsNullOrEmpty(url))
            {
                if (url.Contains("?"))
                {
                    return url + "&" + this.pagerOptions.PageRouteValueKey + "=" + pageNumber;
                }
                else
                {
                    return url + "?" + this.pagerOptions.PageRouteValueKey + "=" + pageNumber;
