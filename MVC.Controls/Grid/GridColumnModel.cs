...
namespace MVC.Controls.Grid
{
    ...
    {
        DEFAULT, INT, FLOAT, DATE, NONE, CUSTOM, DECIMAL
    }
    ...
    public class GridColumnModelList<T> where T : class
    {
        ...
        //لود ستون های فیلتر شده
        public List<GridColumnModel> ItemsFiltered(string selectedColumnNames)
        {
            if (string.IsNullOrWhiteSpace(selectedColumnNames))
                return Items;

            var columnNames=selectedColumnNames.Split(',');
            var selectedColumnModels = new List<GridColumnModel>();
            foreach (var columnName in columnNames)
            {
                var columnModel = Items.FirstOrDefault(a => a.Name == columnName);
                if(columnModel!=null)
                selectedColumnModels.Add(columnModel);
            }
        ...
        public GridColumnModel Add(string Name)
        {
            GridColumnModel column = new GridColumnModel(Name);
            this.Items.Add(column);
            return column;
        }
    ...
    {
        private static readonly Dictionary<GridCellType, string> _columnCellTypes = new Dictionary<GridCellType, string>() { { GridCellType.INT, "int" }, { GridCellType.DATE, "date" }, { GridCellType.FLOAT, "float" }, { GridCellType.DECIMAL, "decimal" } };

        private string _name, _caption, _index, _width, _align, _customAttributes = "";
        private List<GridColumnSearchOption> _searchoptions;
        private GridCellType _cellType = GridCellType.DEFAULT;
        private bool _editable = true, _sortable = true, _hidden = false, _isPrimaryKey = false, _asGroup = false, _searchable = true, _frozen = false;
        ...
        public GridColumnModel SetSearchoptions(List<GridColumnSearchOption> searchoptions)
        { _searchoptions = searchoptions; return this; }
        ...
        public GridColumnModel SetSearchable(bool searchable) { _searchable = searchable; return this; }
        public GridColumnModel SetFrozen(bool frozen) { _frozen = frozen; return this; }
        ...
        public string Render()
        {
            ...
            {
                sb.Append("width: ");
                sb.Append(_width).AppendLine(", ");
            }
            //  sb.Append(", onchange:\"alert('sss')\"");
            //sb.Append(_width);
            ...
            if (!_searchable)
            {
                sb.Append(",search: " + _searchable.ToString().ToLower());
            }
            if (_frozen)
            {
            ...
            if (_name == "act")
            {
                sb.Append(", sortable:false,search:false ");
            }
            //if (_cellType != GridCellType.DECIMAL)
            if (_colRenderer is NumberColumnRenderer || _cellType == GridCellType.DECIMAL ||
                _cellType == GridCellType.FLOAT || _cellType == GridCellType.INT)
            {
                sb.Append(", searchoptions:{sopt:['eq']}");
            }
            else
            {
                //sb.Append(", searchoptions:{sopt:['eq','ne','le','lt','gt','ge']}");
            }
        ...
        internal string renderDataEvents()
        {
            ...
            foreach (string eventName in _clientEvents.Keys)
                sb.Append("{readonly:'readonly',type: '" + eventName + "', fn:function(e){" + _clientEvents[eventName] + "(e);}},");
    ...
    public class GridColumnSearchOption
    {
