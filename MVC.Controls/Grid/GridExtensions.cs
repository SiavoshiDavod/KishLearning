...
namespace MVC.Controls.Grid
{
    public static class GridExtensions
    {
        ...
        public static string Grid(GridControl grid)
        {
            ...
            //sb.AppendLine("\tfunction enterToTab(event) {");
            //sb.AppendLine("\t\tvar key = event.keyCode");
            //sb.AppendLine("\t\tif (key == 13) {");
            //sb.AppendLine("\t\t\tevent.keyCode = 9;");
            //sb.AppendLine("\t\t\treturn true;");
            //sb.AppendLine("\t\t}");
            //sb.AppendLine("\t\treturn false;");
            //sb.AppendLine("\t}");
            if (grid.GetKeepSelectedRows())
            {
                if(string.IsNullOrEmpty(grid.GetKeepSelectedRowIds()))
                    sb.AppendLine("var ").Append(grid.Name).AppendLine("_SelectedRowIds={};");
                else
                {
                    sb.AppendLine("var ").Append(grid.Name).AppendLine("_SelectedRowIdsMap=new Object();");
                    foreach (var keepSelectedRowId in grid.GetKeepSelectedRowIds().Split(','))
                    {
                        //if (!string.IsNullOrEmpty(keepSelectedRowId))
                         sb.Append(grid.Name).Append("_SelectedRowIdsMap[").Append(keepSelectedRowId).AppendLine("]=true;");
                    }
                    sb.AppendLine("var ").Append(grid.Name).Append("_SelectedRowIds=").Append(grid.Name).AppendLine("_SelectedRowIdsMap;");
                }
            }
            ...
            sb.Append("processGridColumns($('#" + grid.Name + "'));");
            sb.Append("restoreGridColumns($('#" + grid.Name + "'));");

            if (grid.IsFilterToolbar)
            {
                if (grid.FilterToolbar == null)
                    grid.FilterToolbar = new FilterToolbar();

                sb.Append("$('#").Append(grid.Name).Append("').jqGrid('filterToolbar', { stringResult: ").Append(grid.FilterToolbar.StringResult?"true":"false").
                    Append(", searchOnEnter: ").Append(grid.FilterToolbar.SearchOnEnter ? "true" : "false").Append(", defaultSearch: '").Append(grid.FilterToolbar.DefaultSearch).
                    Append("', ignoreCase: ").Append(grid.FilterToolbar.IgnoreCase ? "true" : "false").AppendLine(" });");
            }
        ...
        /// <returns></returns>
        public static MvcHtmlString GridSaveButton(this HtmlHelper htmlHelper, string buttonName = "btnSave", string buttonText = "save", string gridName = "grid", IDictionary<string, object> htmlAttributes = null, string clientEvent = null, bool allRows = false, string bulkUrl = null, string parameterName = null,string afterSaveFunction = null)
        {
            StringBuilder sb = new StringBuilder();
            ...
            else
            {
               ...
               // sb.AppendFormat("$(\"#{1}\").jqGrid('saveRow', {0}, aftersavefunc=updateButtonState($('#{1}')));\r\n", GridSelectedRow(htmlHelper, gridName), gridName);
                if (!string.IsNullOrEmpty(afterSaveFunction))
                {
                    sb.AppendFormat("$(\"#{1}\").jqGrid('saveRow', {0}, aftersavefunc=updateButtonState($('#{1}')));\r\n", GridSelectedRow(htmlHelper, gridName), gridName);
                }
                sb.AppendFormat("$(\"#{1}\").jqGrid('saveRow', {0});\r\n", GridSelectedRow(htmlHelper, gridName), gridName);
            }
            sb.AppendLine("});");
            ...
            return new MvcHtmlString(tagBuilder.ToString() + sb.ToString());
        }
        ...
        /// <returns></returns>
        public static MvcHtmlString GridUpdateButton(this HtmlHelper htmlHelper, string buttonName = "btnUpdate", string buttonText = "Update", string gridName = "grid", string clientEvent = null, IDictionary<string, object> htmlAttributes = null, string rowNum = null)
        {
        ...
        /// <returns></returns>
        public static MvcHtmlString GridCancelButton(this HtmlHelper htmlHelper, string buttonName = "btnCancel", string buttonText = "Cancel", string gridName = "grid", IDictionary<string, object> htmlAttributes = null, string clientEvent = null, bool allRows = false)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("<script language=\"javascript\" type=\"text/javascript\">");
            sb.AppendLine("$(\"#" + buttonName + "\").click(function() {");
        ...
        /// <param name="htmlAttributes">Additional html attributes for the button</param>
        /// <returns></returns>
        public static MvcHtmlString GridAddButton(this HtmlHelper htmlHelper, string buttonName = "btnAdd", string buttonText = "Add", string gridName = "grid", string clientEvent = null, IDictionary<string, object> htmlAttributes = null)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("<script language=\"javascript\" type=\"text/javascript\">");
            sb.AppendLine("$(\"#" + buttonName + "\").click(function() {");
        ...
        /// <param name="htmlAttributes">Additional html attributes for the button</param>
        /// <returns></returns>
        public static MvcHtmlString GridDeleteButton(this HtmlHelper htmlHelper, string buttonName = "btnDelete", string buttonText = "Delete", string gridName = "grid", string actionUrl = null, IDictionary<string, object> htmlAttributes = null, string clientEvent = null)
        {
            StringBuilder sb = new StringBuilder();
            sb.AppendLine("<script language=\"javascript\" type=\"text/javascript\">");
            sb.AppendLine("$(\"#" + buttonName + "\").click(function() {");
           ...
           // sb.AppendFormat(update); //by our changes &&&&&&&&&&&
