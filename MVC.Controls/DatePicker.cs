...
namespace MVC.Controls
{
    public class DatePicker
    {
        ...
        public bool IsMask
        {
            get
            {
                if (_attributes.ContainsKey("isMask"))
                    return bool.Parse(_attributes["isMask"]);
                else return false;
            }
            set { _attributes["isMask"] = value.ToString().ToLower(); }
        }
        /// <summary>
        ...
        public string Render(string id)
        {
            ...
            if (IsMask)
            {
                sb.Insert(0, "$(\"#" + id + "\").mask(\"9999/99/99\", { placeholder: \"_\" });");
            }
            else
            {
