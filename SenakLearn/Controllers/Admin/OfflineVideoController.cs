...
namespace SenakLearn.Controllers.Admin
{
    public class OfflineVideoController : BaseAdminController
    {
        ...
        public ActionResult IndexPartial(int learn_coursId)
        {
            ViewBag.Url = "/OfflineVideo/GetTreeList?id=" + learn_coursId;
            ViewBag.learn_coursId = learn_coursId;

            return PartialView("~/Views/OfflineVideo/IndexPartial.cshtml", new OfflineVideo { learn_coursId=learn_coursId});
        }
        ...
        {
            var OfflineVideo = Biz.OfflineVideoBiz.Instance.GetAllVideo(id);// db.OnlineClassAccorations.Find(id);
            if (OfflineVideo == null)
            {
                return HttpNotFound();
            }
            var treeList = GetRecursiveJsTreeList<OfflineVideoWrapper>.Instance.GetTreeList(OfflineVideo.ToList());
        ...
        public ActionResult CreateByParent(int parentId)
        {
            if (parentId != 0)
            {
                var parentVideo = Biz.OfflineVideoBiz.Instance.Get(parentId);
                var model = new OfflineVideo { ParentId = parentId, ParentName = parentVideo?.Description };
                return PartialView("~/Views/OfflineVideo/Create.cshtml", model);
            }
            return PartialView("~/Views/OfflineVideo/Create.cshtml");
        }
        [HttpPost]
        public ActionResult Create(OfflineVideo OfflineVideo)
        {
            OfflineVideo.Validate();
            //  if (ModelState.IsValid)
            {
                //if(OfflineVideo.VideoId==new Guid("00000000-0000-0000-0000-000000000000"))
