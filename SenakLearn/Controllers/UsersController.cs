...
namespace SenakLearn.Controllers
{
    public class UsersController : BaseController
    {
        ...
        public ActionResult Register(string ReturnUrl)
        {
            ...
            orgs.Insert(0, new Organization { Title = "..." });
            ViewBag.listOrg = orgs.Select(a => new SelectListItem { Value = a.Id.ToString(), Text = a.Title, Selected = false }).ToList();
            var posts = PostBiz.Instance.GetAll().ToList();
            posts.Insert(0, new Post { Title = "سمت" });
            ViewBag.listPost = posts.Select(a => new SelectListItem { Value = a.Id.ToString(), Text = a.Title, Selected = false }).ToList();
        ...
        public async Task<ActionResult> Register(RegisterViewModel register, string ReturnUrl = "/")
        {
            ...
            try
            {
                ...
                //    return View();
                //}
                if (register.OrgId == 0) register.OrgId = null;
                if (register.PostId == 0) register.PostId = null;
        ...
        {
            var model = GetSessionUser();
            if (model.id != 0)
            {
                var sumCourses = PersonCertificateBiz.Instance.GetCourseDuration(model.id, null);
                var yearPersian = DateTime.Now.GeogianToPersianString().Substring(0, 4).ToPersianNumber();
                var sumCoursesInYear = PersonCertificateBiz.Instance.GetCourseDuration(model.id, yearPersian);
                model.CourseDurationSum = sumCourses;
                model.CourseDurationYear = sumCoursesInYear;
                if(model.OrgId!=null)
                {
                    model.OrgName = OrgBiz.Instance.Get(model.OrgId.Value).Title;
                }
            }
            return View(model);
        }
        ...
        public ActionResult Login(string ReturnUrl)
        {
            ...
            if (ReturnUrl != null)
            {
                ...
                SetViewBagErrorMessage(" کاربر محترم قبل از ادامه لازم است وارد شوید یا ثبت نام نمایید !");
            }
        ...
        [HttpPost]
        public JsonResult Upload(int OrgId)
        {
            var result = UserBiz.Instance.UploadUser(Request.Files[0],OrgId);
