namespace AdobeConnectSDK.Extensions
{
    ...
    public static class PrincipalManagement
    {
        ...
        public static PrincipalInfoStatus GetPrincipalInfo(this AdobeConnectXmlAPI adobeConnectXmlApi, string principalId)
        {
            ...
            {
                throw new Exception("Adobe-Extensions-PrincipalManagement-GetPrincipalInfo", ex.InnerException ?? ex);
        ...
        /// </returns>
        public static EnumerableResultStatus<PrincipalListItem> GetPrincipalList(this AdobeConnectXmlAPI adobeConnectXmlApi, string filterBy)
        {
            ApiStatus s = adobeConnectXmlApi.ProcessApiRequest("principal-list", filterBy);
            ...
            {
                throw new Exception("Adobe-Extensions-PrincipalManagement-GetPrincipalList", ex.InnerException ?? ex);
                //throw ex.InnerException;
            }
        ...
        {
            return adobeConnectXmlApi.GetPrincipalList(String.Empty);
        }
        ...
        public static EnumerableResultStatus<PermissionInfo> GetPermissionsInfo(this AdobeConnectXmlAPI adobeConnectXmlApi, string aclId, string principalId, string filter)
        {
            ...
            {
                throw new Exception("Adobe-Extensions-PrincipalManagement-GetPermissionsInfo", ex.InnerException ?? ex);
                //throw ex.InnerException;
            }
        ...
        public static PermissionInfoByPermissionIdStatus GetPermissionsInfoByprincipalId(this AdobeConnectXmlAPI adobeConnectXmlApi, string aclId, string principalId)
        {
            ApiStatus s = adobeConnectXmlApi.ProcessApiRequest("permissions-info", String.Format("acl-id={0}&principal-id={1}", aclId, principalId));

            var resultStatus = Helpers.WrapBaseStatusInfo<PermissionInfoByPermissionIdStatus>(s);
            ...
            try
            {
                XElement nodeData = s.ResultDocument.XPathSelectElement("//permission");

                if (nodeData == null)
                {
                    return null;
                }
                if (nodeData.HasAttributes)
                {
                    resultStatus.Result = new PermissionInfoByPermissionId();
                    foreach (var item in nodeData.Attributes())
                    {
                        if (item.Name == "permission-id")
                        {
                            resultStatus.Result.PermissionIdRaw = item.Value;
                        }
                        else if (item.Name == "acl-id")
                        {
                            resultStatus.Result.AclId = item.Value;
                        }
                        else if (item.Name == "principal-id")
                        {
                            resultStatus.Result.PrincipalId = item.Value;
                        }
                        else
                        {
                            return null;
                ...
                //  resultStatus.Result = XmlSerializerHelpersGeneric.FromXML<PermissionInfoByPermissionId>(nodeData.CreateReader());
                //var userInfo = XmlSerializerHelpersGeneric.FromXML<PermissionInfoByPermissionId>(s.ResultDocument.Descendants("permission").FirstOrDefault().CreateReader());
                // userInfoStatus.Result = userInfo;
                return resultStatus;
            }
            catch (Exception ex)
            {
                throw new Exception("Adobe-AdobeConnectXmlAPI-UserInfo", ex.InnerException ?? ex);
                // throw ex.InnerException;
            }
