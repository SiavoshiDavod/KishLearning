...
pw.jqXHRresponseText = function (responseText) {
    ...
    $.ajax({
        ...
        success: function (result) {
            if (addToHistory === true) {
                pw.changeUrl(window.location.origin + 'url=' + url, title);
            }
    ...
    $.ajax({
        ...
        complete: function (jqXHR) {
            if (jqXHR.state == 401) {
                ...
                pw.jqXHRresponseText(jqXHR.responseText);
            }
        },
        timeout: 300000
    });
    ...
    $.ajax({
        ...
        success: function (response) {
            if (response.state === false) {
                pw.SwalErrorMessage(response.ErrorMessage);
                ...
                pw.SwalSuccessMessage(response.Message);
                //if (response.getMessage == '')
                //    pw.notificationSuccess('عملیات با موفقیت انجام شد.');
                //else
                //    pw.notificationSuccess(response.Title, response.getMessage);
...
pw.SwalErrorMessage=function (message) {
    Swal.fire({
        position: 'top-end',
        type: 'error',
        title: 'خطا',
...
pw.SwalSuccessMessage=function (message) {
    if (message == '') {
        Swal.fire({
            position: 'top-end',
            type: 'success',
