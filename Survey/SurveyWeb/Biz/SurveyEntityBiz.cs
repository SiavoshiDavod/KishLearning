    ...
    public class SurveyEntityBiz : RepositoryBase<Models.SurveyEntity>
    {
        ...
        public async Task<SurveyEntity> GetIncludeQuestion(int id, string ip, int userId)
        {
            using (var ctx = new Context())
            {
                ...
                {
                    throw new HandledException(" شما قبلا در این نظر سنجی شرکت کرده اید");
                }
                ...
                if (obj.IsUserMustBeLogin)
                {
                    if (userId <= 0)
                    {
                        throw new HandledException(" لطفا ابتدا در سیستم لاگین کنید");
                    }
                ...
                {
                    throw new HandledException("شما دسترسی برای شرکت در این نظرسنجی را ندارید");
                }
