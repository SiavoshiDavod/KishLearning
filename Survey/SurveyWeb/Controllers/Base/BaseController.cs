...
namespace SurveyWeb.Controllers
{
    ...
    {
        protected string ip => HttpContext.Request.UserHostAddress;
        ...
        internal void CheckGoogleRecapcha(string gRecaptchaResponse)
        {
            ...
            {
                throw new HandledException("خطا در اعتبار سنجی ، لطفا مجددا تلاش کنید", "/Home/RegisterOrLogin");
            }
            ...
            //{
            //    throw new HandledException("خطا در اعتبار سنجی گوگل، لطفا مجددا تلاش کنید");
            //    //throw new HandledException( "Google recaptcha error: "+string.Join(" , ", captChaesponse.ErrorCodes));
            //}
        }
        ...
        internal string SaveFile(HttpPostedFileBase ImageFile, pathFile path,string id="")
        {
            string img = ImageFile?.FileName.Split('.')[0] + id + "." + ImageFile?.FileName.Split('.')[1];
            if (ImageFile != null)
            {
                img = img.Replace(" ", "").Replace("%", "").Replace("&", "").Replace("#", "");
                //img = Guid.NewGuid().ToString().Replace("-", "") + id + "." + ImageFile?.FileName.Split('.')[1];
                if (System.IO.File.Exists(Server.MapPath("/images/" + path + "/" + img)))
        ...
        protected override void OnActionExecuting(ActionExecutingContext filterContext)
        {
            ...
            ViewBag.AttachFileTitle = " فقط فایل های حداکثر با اندازه 500کيلوبايت بارگزاري کنيد!";
        ...
        protected override void OnException(ExceptionContext filterContext)
        {
            ...
            if (Request.IsAjaxRequest())
            {
                ...
                if (isJson)
                {
                    ...
                    {
                        Data = new ApiJsonResult { success = false, ErrorMessage = innerEx.Message, InnerExceptionMessage = ExceptionExtensions.GetStackTraceWithMessage(innerEx) },
            ...
            else
            {
                ...
                //
                //filterContext.ExceptionHandled = true;
                if (innerEx is HandledException)
                {
                    HandledException ex = (HandledException)innerEx;
                    if (string.IsNullOrEmpty(ex.TargetUrl))
                    {
                        filterContext.Controller.ViewData.ModelState.AddModelError("", ex.Message);
                        //filterContext.Controller.ViewData.Model = filterContext.HttpContext.Request.Form ;
                        if (filterContext.HttpContext.Request.Url.PathAndQuery == "/Resturant/Create")
                            filterContext.Result = new RedirectResult("/Resturant/Index");
                        else
                        filterContext.Result = new RedirectResult(filterContext.HttpContext.Request.Url.PathAndQuery);
                    }
                    else
                    {
                        filterContext.Result = new RedirectResult(ex.TargetUrl);
                    }
                }
                else
                {
        ...
        protected void SetLog(Exception e)
        {
            try
            {
                ...
                {
                    streamWriter.WriteLine(System.DateTime.Now.ToString() + " " + e.Message + " " + ExceptionExtensions.GetStackTraceWithMessage(e));
        ...
        public static ActionResult PrintListToExcel<T>(List<T> list, List<GridColumnModel> gridColumnModelList, string worksheetCaption, string gridHeader, int? decimalplaces, HttpResponseBase response, bool? showSumRow = null, bool? showRowNumber = null, bool? hasFilter = null, bool? setBlueOneLastBackgroundColorRow = null, bool rtl = true)
        {
            ...
            if (!list.Any())
                throw new HandledException("رکوردی یافت نشد");
